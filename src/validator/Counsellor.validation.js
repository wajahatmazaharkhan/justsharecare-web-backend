...
export const CounsellorValidation = z.object({
    ...
    specialties: z.string().min(2, "spaciality is important"),

    bio: z.string().min(2, "bio is important"),

    qualifications: z.string().min(2, "qualification is important"),
    ...
    calendar_integration: z.string(),

    session_type: z
        .string()
        .transform(val => val.split(",").map(s => s.trim()))  // split comma-separated string
        .refine(arr => arr.every(v => ["video", "audio", "chat"].includes(v)), {
